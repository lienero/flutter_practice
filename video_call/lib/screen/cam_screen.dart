...
class _CamScreenState extends State<CamScreen> {
  ...
  Future<bool> init() async { // 권한 관련 작업 모두 실행
    ...
    if (engine == null) {
      ...
      engine!.registerEventHandler(
        RtcEngineEventHandler( // 아고라 엔진에서 받을 수 있는 이벤트 값들 등록
          onJoinChannelSuccess: (RtcConnection connection, int elapsed) {
            // 채널 접속에 성공했을 때 실행
            print('채널에 입장했습니다. uid : ${connection.localUid}');
            setState(() { this.uid = connection.localUid; });
          },
          onLeaveChannel: (RtcConnection connection, RtcStats stats) {
            print('채널 퇴장'); // 채널을 퇴장했을 때 실행
            setState(() { uid = null; });
          },
      ...
      await engine!.enableVideo(); // 동영상 기능을 활성화 합니다.
      await engine!.startPreview(); // 카메라를 이용해 동영상을 화면에 실행합니다.
      await engine!.joinChannel( // 채널에 들어가기
        token: TEMP_TOKEN, // 채널 입장하기
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder( // Future 기반으로 위젯 렌더링
        future: init(),
          ...
          return Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: ElevatedButton( // 뒤로가기 기능 및 채널 퇴장 가능
                  onPressed: () async {
                    if (engine != null) {
  ...
  Widget renderSubView() {
    if (uid != null) {
      // AgoraVideoView 위젯을 사용하면, 동영상을 화면에 보여주는 위젯을 구현가능
      ...
      // 아직 내가 채널에 접속하지 않았다면 로딩 화면을 보여줍니다.
    ...
    if (otherUid != null) {
      return AgoraVideoView(
        ...
        controller: VideoViewController.remote(
          rtcEngine: engine!,
          canvas: VideoCanvas(uid: otherUid), // uid에 상대방 ID를 입력해줍니다.
          connection: const RtcConnection(channelId: CHANNEL_NAME),
        )
      );
    } else {
      // 상대가 아직 채널에 들어오지 않았다면 대기 메세지를 보여줍니다.
      return Center(
        child: const Text(
